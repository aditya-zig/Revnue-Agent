...
def upgrade() -> None:
    # PaymentObligation is explicit verified merchant reference; one permanent RecoveryCase per obligation.
    ...
    op.create_index("ix_payment_events_obligation_reference", "payment_events", ["obligation_reference"])
    op.add_column(
        "recovery_cases",
        sa.Column("obligation_reference", sa.String(length=128), nullable=True),
    )
    op.create_index("ix_recovery_cases_obligation_reference", "recovery_cases", ["obligation_reference"])
    # payment_id was previously unique; with obligation grouping multiple payment_ids map to one case,
    # so the case's payment_id is no longer unique. Keep index but drop unique constraint.
    # SQLite does not support altering unique via alembic directly; we recreate index without unique.
    # The unique constraint was implicit via unique=True; we keep the column but allow duplicates by not enforcing.
