...
**RecoveryCase**:
A workflow object for one payment_id that owns state, decisions, actions, and outcome.
_Avoid_: Ticket, case file, recovery

**PaymentException**:
A workflow for a payment reported as failed when the Customer may also have been debited, kept separate from the RecoveryCase that asks for payment again.
