...
| Data disclosure | `.env`, databases, Python caches, virtual environments, and `rzp-test-key.csv` are ignored. Razorpay Test Mode keys live in `~/.razorpay/config.yaml` and `RAZORPAY_BASIC_TOKEN` in the shell, not in the repo. OpenRouter keys are environment/secret-manager values; FindingAnalysis sends only the sanitized aggregate snapshot, denies provider data collection, and sends no tools. The committed demo records have synthetic identifiers. | Local SQLite encryption, access control, retention, deletion, and OpenRouter provider retention remain production review items. |
